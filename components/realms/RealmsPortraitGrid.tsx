...
const GOLD = '#c9973a'
const COLS = 7 // cards per row — strip inserts after each row
...
function RealmCard({
  world, cardImage, hovered, onEnter,
}: {
  ...
  return (
    <Link href={`/realms/${world.slug}`}>
      ...
      >
        {/* Ground glow — pool of light under the card */}
        <motion.div
          className="absolute inset-x-2 pointer-events-none"
          style={{
            bottom: -14,
            height: 18,
            background: `radial-gradient(ellipse at 50% 0%, ${color}90, transparent 70%)`,
            filter: 'blur(6px)',
          }}
        ...
        <div className="absolute inset-0" style={{
          background: hovered
            ? 'linear-gradient(to top, rgba(6,4,12,0.92) 0%, rgba(6,4,12,0.1) 55%, transparent 100%)'
            : 'linear-gradient(to top, rgba(6,4,12,0.88) 0%, rgba(6,4,12,0.18) 50%, transparent 100%)',
        }} />
        {hovered && (
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-2.5">
          ...
          <p className="text-[7px] mt-0.5 tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.75)' }}>Enter →</p>
        </div>
        ...
        <div className="absolute bottom-2 right-2 w-3 h-3 pointer-events-none" style={{ borderBottom: `1px solid ${color}30`, borderRight: `1px solid ${color}30` }} />
      </motion.div>
    </Link>
  )
}

// ── Persistent info strip — always rendered, content updates on hover ─────────
  ...
  return (
    ...
    >
      ...
      {world ? (
        <>
          {/* Title */}
          <p className="font-cinzel text-sm font-bold tracking-widest shrink-0 whitespace-nowrap"
          ...
          {/* Description */}
          <p className="flex-1 text-xs truncate" style={{ color: 'rgba(255,255,255,0.88)' }}>
            {world.short_description}
          </p>

          {/* CTA */}
          <Link href={`/realms/${world.slug}`} className="shrink-0">
            <motion.span
              className="inline-flex items-center gap-1.5 px-4 py-1 rounded text-[10px] tracking-widest uppercase font-semibold cursor-pointer whitespace-nowrap"
              style={{
              ...
              whileHover={{ scale: 1.04, boxShadow: `0 4px 18px ${GOLD}30` }}
              whileTap={{ scale: 0.97 }}
...
export default function RealmsPortraitGrid({ worlds, cardImages }: Props) {
  const [hoveredSlug, setHoveredSlug] = useState<string | null>(null)

  // Split into rows of COLS
  const rows: Partial<IpWorld>[][] = []
  for (let i = 0; i < worlds.length; i += COLS) {
    rows.push(worlds.slice(i, i + COLS))
  }
    ...
    <div className="px-6 pt-6 pb-10" style={{ background: '#08060e' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-2">
          <p className="text-[9px] tracking-[0.5em] uppercase mb-2" style={{ color: `${GOLD}90` }}>
            Select a Realm
          </p>
        ...
        {/* Rows with persistent strip between each */}
        {rows.map((row, rowIdx) => {
